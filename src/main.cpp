...
#if defined(NDEBUG)
# error "Runner cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "Runner Signed Message:\n";
...
bool ReadBlockFromDisk(CBlock& block, const CDiskBlockPos& pos)
{
    ...
    // Check the header
    LogPrintf("ReadBlockFromDisk ...Check the header \n");
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck)
{
    ...
    if (!CheckBlock(block, state, !fJustCheck, !fJustCheck)){
        LogPrintf("ConnectBlock ..CheckBlock false!\n");
        return false;
    }

    LogPrintf("ConnectBlock ..verify that the view's current state corresponds to the previous block\n");
...
bool CheckBlockHeader(const CBlockHeader& block, CValidationState& state, bool fCheckPOW)
{
    ...
    if (fCheckPOW && !CheckProofOfWork(block.GetPoWHash(), block.nBits)){
        ...
        LogPrintf("CheckBlockHeader ...hash doesn't match nBits\n");
        return state.DoS(50, error("CheckBlockHeader() : proof of work failed"),
                         REJECT_INVALID, "high-hash"); //错误
    }

    // Check timestamp
    if (block.GetBlockTime() > GetAdjustedTime() + 2 * 60 * 60) //检查时间戳..
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot)
{
    ...
    LogPrintf("CheckBlock: CheckBlockHeader begin 2\n");// has bug
    if (!CheckBlockHeader(block, state, fCheckPOW)){
        LogPrintf("CheckBlock: CheckBlockHeader false\n");// has bug
        return false;
    }
...
bool ContextualCheckBlockHeader(const CBlockHeader& block, CValidationState& state, CBlockIndex * const pindexPrev)
{
    ...
    // Runner: Reject block.nVersion=1 blocks (mainnet >= 710000, testnet & regtest uses supermajority)
...
bool ContextualCheckBlock(const CBlock& block, CValidationState& state, CBlockIndex * const pindexPrev)
{
    ...
    // Runner: (mainnet >= 710000, testnet >= 400000, regtest uses supermajority)
    // Enforce block.nVersion=2 rule that the coinbase starts with serialized block height
    // if 750 of the last 1,000 blocks are version 2 or greater (51/100 if testnet):
    bool checkHeightMismatch = false;
    if (block.nVersion >= 18)
    {
        if (Params().EnforceV2AfterHeight() != -1)
        {
...
bool AcceptBlockHeader(const CBlockHeader& block, CValidationState& state, CBlockIndex** ppindex)
{
    ...
    LogPrintf("CheckBlockHeader begin 1..\n");
...
bool static LoadBlockIndexDB()
{
    ...
    if (it == mapBlockIndex.end()){
        LogPrintf("LoadBlockIndexDB(): is end \n");
        return true;
    }
    chainActive.SetTip(it->second);
    LogPrintf("LoadBlockIndexDB(): PruneBlockIndexCandidates \n");
    PruneBlockIndexCandidates();
